...
# Every model id typed in the panel's "OpenRouter models" box (NOT just the
# ticked ones). Written at deploy from the vault and refreshed by fbpool.py
# every 60s. Used to (a) recognise a requested model as OpenRouter even
# when its id has no ":free" suffix (e.g. "stealth/ox-alpha") and (b) show
# those models in /v1/models so Hermes' /model picker lists them.
OR_LIST_FILE = os.path.join(HOME, "or_models.txt")

def or_models():
    out, seen = [], set()
    try:
        raw = open(OR_LIST_FILE).read()
    except Exception:
        raw = os.environ.get("OR_MODELS", "")
...
def candidates(model):
    pool = load_pool()
    if model == "or-auto":
        # ── separate OpenRouter pool (like pool-auto, but OR-only) ──
        # Picked from /model under the "openrouter-free" provider. Rotates
        # across EVERY model in the panel's OpenRouter box, regardless of
        # route_mode. Never touches NVIDIA/Mistral.
        if not KEYS.get("openrouter"):
            return []
        ors = [x["id"] for x in pool if x["provider"] == "openrouter"]
        for m in or_models():
            if m not in ors:
                ors.append(m)
        return [("openrouter", m) for m in ors]
    if model in ("", None, "pool-auto"):
        ...
        if route_mode() == "openrouter":
            ...
            ors = [x["id"] for x in pool if x["provider"] == "openrouter"]
            for m in or_models():   # panel textarea models count too
                if m not in ors:
                    ors.append(m)
            return [("openrouter", m) for m in ors]
    ...
    if model.startswith(mist):
        prov = "mistral"
    elif KEYS.get("openrouter") and (model.endswith(":free")
                                     or model in or_models()):
        # ":free" suffix OR any model listed in the panel's OpenRouter box
        # (ids like "stealth/ox-alpha" carry no suffix - the old suffix-only
        # check silently sent them to NVIDIA, which 404'd)
        prov = "openrouter"
    else:
        prov = "nvidia"
    return [(prov, model)]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/models", "/v1/models"):
            ...
            # separate OpenRouter pool + panel's OpenRouter list, so /model
            # shows YOUR added models and the or-auto rotation entry
            if KEYS.get("openrouter"):
                ids.insert(1, "or-auto")
                ids += [m for m in or_models() if m not in ids]
