...
Private messages (user-to-user DM threads):
  ...
  hermesa-group call <userIdOrName> [seconds]      # RING the user's phone: realtime
                                                   # full-screen incoming DM call (app v5+)
...
DEFAULT_GROUP_DB_URL = ""  # no baked-in endpoint - the config panel (vault) supplies the URL
MAX_CHUNK = 3500
...
def db_url():
    u = env("HERMESA_GROUP_DB_URL", DEFAULT_GROUP_DB_URL).rstrip("/")
    if not u:
        print("ERROR: HERMESA_GROUP_DB_URL is not set - open the config "
              "panel (index.html) and press Apply once; it fills this "
...
_BOT_NAME_CACHE = None


def bot_name():
    """Display name for the bot. Prefers the LIVE name the owner set in the
    app's bot settings (group/users/<owner>.botName) so CLI posts show the
    SAME name as the bridge, then HERMESA_GROUP_BOT_NAME, then "Hermes"."""
    global _BOT_NAME_CACHE
    if _BOT_NAME_CACHE:
        return _BOT_NAME_CACHE
    try:
        data = get("group/users/%s" % owner_id()) or {}
        name = (data.get("botName") or "").strip()
        if name:
            _BOT_NAME_CACHE = name
            return name
    except Exception:
        pass
    _BOT_NAME_CACHE = env("HERMESA_GROUP_BOT_NAME", "Hermes")
    return _BOT_NAME_CACHE
...
def cmd_call(peer, user, seconds):
    """Rings the peer's phone through the app's realtime call system.

    Writes the bot in as a participant of the DM call thread; app builds v5+
    ring a full-screen incoming call on the peer's phone (even locked). The
    bot has NO live WebRTC audio - after ringing, deliver the actual message
    with `dm-voice` (TTS mp3) or `dm` text.
    """
    owner = owner_id()
    dm = dm_id_for(owner, peer)
    me = "bot:%s" % owner
    path = "group/calls/%s/participants/%s" % (dm, me)
    ...
    print("ringing %s (%s) for up to %ds ..." % (user.get("name") or peer, peer, seconds))
    joined = False
    waited = 0
    while waited < seconds:
        time.sleep(3)
        waited += 3
        parts = get("group/calls/%s/participants" % dm) or {}
        if any(k != me for k in parts):
            joined = True
            break
    http("DELETE", "%s/%s.json" % (db_url(), path))
    if joined:
        print("ok: %s answered. NOTE: the bot has no live microphone - now "
              "send `dm-voice` (TTS mp3) or `dm` text to talk." % peer)
    else:
        send_text(dm, "📞 Missed call from %s" % bot_name())
...
def main():
    ...
    if cmd == "text":
        ...
        if not rest:
            print("ERROR: call requires <userIdOrName>", file=sys.stderr)
            sys.exit(2)
