...
def send_image(image_url_or_path: str, text: str = "Image", file_name: str = "image.png") -> None:
    """Send an image. Accepts an https:// URL or a local file path (auto Base64)."""
    image_url = image_url_or_path
    if os.path.isfile(image_url_or_path):
        ext = os.path.splitext(image_url_or_path)[1].lstrip(".").lower() or "png"
...
if __name__ == "__main__":
    ...
    p = sub.add_parser("file", help="send a file attachment by URL")
    p.add_argument("url")
