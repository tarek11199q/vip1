...
# The app ships with a built-in Firebase server (same constant as the
# bridge and the app's FirebaseManager.kt). When the panel leaves
# HERMESA_DB_URL empty we fall back to it, so text / images / VOICE CALLS
# work out of the box with just HERMESA_BOT_ID set.
DEFAULT_APP_DB_URL = "https://softizence-default-rtdb.firebaseio.com"
...
def _post(payload: dict) -> None:
    if not BOT_ID:
        raise SystemExit("Hermesa not configured - set HERMESA_BOT_ID in the "
                         "config panel (HERMESA_DB_URL is optional: leave it "
                         "empty to use the app's built-in server).")
