...
# No baked-in endpoint in the repo: the database URL comes ONLY from the
# config panel (index.html -> Firebase vault). When HERMESA_DB_URL is left
# empty we fall back to the panel-supplied group server URL, so text /
# images / VOICE CALLS work with just HERMESA_BOT_ID set.
DEFAULT_APP_DB_URL = _cfg("HERMESA_GROUP_DB_URL", "")
BOT_ID = _cfg("HERMESA_BOT_ID", "")
DATABASE_URL = (_cfg("HERMESA_DB_URL", "") or DEFAULT_APP_DB_URL).rstrip("/")
...
def _post(payload: dict) -> None:
    if not BOT_ID or not DATABASE_URL:
        raise SystemExit("Hermesa not configured - set HERMESA_BOT_ID in the "
                         "config panel and press Apply once (the panel "
                         "supplies the database URL automatically).")
