...
# No baked-in endpoint: HERMESA_GROUP_DB_URL comes ONLY from the config
# panel (index.html -> Firebase vault); the bridge idles until it is set.
DEFAULT_GROUP_DB_URL = ""
...
def should_reply(m: dict, owner: str, bid: str, bname: str,
    ...
    if m.get("senderIsBot"):
        ...
        if not (mentioned or task):
            return False
        # loop guard: pause a bot<->bot ping-pong only after N consecutive
        # bot messages SINCE THE LAST HUMAN message. Any human message in
        # between resets the counter, so conversations resume naturally.
        try:
            limit = int(os.environ.get("HERMESA_BOT2BOT_MAX", "12") or 12)
        except ValueError:
            limit = 12
        run = 0
        for x in reversed(recent):
            if x.get("senderIsBot"):
                run += 1
            else:
                break
        if run >= limit:
            log("bot-to-bot loop guard: %d bot msgs in a row (limit %d) - "
                "waiting for a human to speak" % (run, limit))
            return False
        return True
    if bot_dm:
...
def build_prompt(m: dict, gid: str, gname: str, group: dict, users: dict,
    ...
    lines.append("You are running inside your normal Hermes workspace with "
                 "FULL access to your skills, memory/brain files, scripts "
                 "and tools - read and use them exactly like any other task.")
...
def process_message(db: str, gid: str, gname: str, group: dict, owner: str,
    ...
    bid = bot_member_id(owner)
    # Snapshot the bot's EXISTING message ids BEFORE the agent runs, so the
    # double-reply guard below works by id and is immune to clock skew
    # between this machine and Firebase server timestamps (the CLI posts
    # with {".sv":"timestamp"}, which can land BEFORE our local clock).
    try:
        pre_ids = set(x.get("id") for x in fetch_messages(db, gid)
                      if x.get("senderId") == bid)
    except Exception:
        pre_ids = set()
    try:
    ...
    # DOUBLE-REPLY GUARD: if the agent ALREADY posted into this chat via the
    # CLI while handling this message, skip posting its final narration text
    # too - otherwise the group sees the same answer twice.
    try:
        already = [x for x in fetch_messages(db, gid)
                   if x.get("senderId") == bid
                   and x.get("id") not in pre_ids
                   and x.get("id") not in st["my_ids"]]
    except Exception:
        already = []
    if already:
        for x in already:
            if x.get("id"):
                st["my_ids"].append(x["id"])
        touch_dm_index(db, gid, owner)
        log("agent already replied via CLI - skipped duplicate bridge reply")
        return
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            # Keep the CLI's display name in sync with the app's bot
            # settings, so messages the AGENT sends via `hermesa-group`
            # never show up under a different name (e.g. "Hermes" while
            # the app calls the bot "Roslina").
            if bname and bname != last_bname:
                hb.persist_env({"HERMESA_GROUP_BOT_NAME": bname})
                last_bname = bname
